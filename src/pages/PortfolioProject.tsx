...
const PortfolioProject = () => {
    ...
    return (
      <Layout>
        <div className="container-tight px-4 py-20 text-center">
          ...
          <Button asChild>
            <Link to="/portfolio">Back to Our Work</Link>
          </Button>
        </div>
      </Layout>
    );
  ...
  return (
    <Layout>
      <SEOMeta
        title={`${project.clientName} — ${project.industry} Case Study | Digital Edge Studio`}
        description={project.excerpt}
        keywords={`${project.industry.toLowerCase()} website case study, ${project.location.toLowerCase()} web design, ${project.clientName.toLowerCase()} website`}
        canonical={`https://digitaledgestudio.com/portfolio/${project.slug}`}
        ogTitle={project.title}
        ogDescription={project.excerpt}
        ogImage={project.image.startsWith("http") ? project.image : `https://digitaledgestudio.com${project.image}`}
      />
        ...
        <div className="container-tight max-w-3xl">
          <div className="bg-accent/5 rounded-xl p-6">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <span className="text-muted-foreground">•</span>
              <Link to="/blog" className="text-sm text-accent hover:underline">Blog</Link>
            </div>
          </div>
        </div>
      </section>
            ...
            <ScrollReveal>
              ...
              <motion.div variants={fadeUp} className="grid md:grid-cols-3 gap-6">
                {otherProjects.map((other) => (
                  <Link key={other.slug} to={`/portfolio/${other.slug}`} className="group">
